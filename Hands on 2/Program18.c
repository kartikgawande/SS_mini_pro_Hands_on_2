...
int main() {
	...
	if (!fork()) {
		...
		//This child is responsible for running "ls -Rl | grep ^d" command where input is received from read-end of pipe 1 = STDIN 
		//and output is written to write-end of pipe 2 = STDOUT
		if (!fork()) {
			dup2(fd2[1], 1);
			dup2(fd1[0], 0);
			close(fd1[1]);
		...
		// This parent is responsible for running "ls -Rl | grep ^d |	wc" command where input is received from read-end of pipe 2 = STDIN 
